...
def CalculatePlotAndSavePoolPresenceTableTotalReadCountHistogram(poolPresenceTable, rowPools, \
                                                                 colPools, prPools, pcPools, controlPools,
                                                                 cumulativePlotFileName, cumulativePlotFileNameNC, \
                                                                 threshold):
...
def CalculateScoreHistogramForSingleAddressLines(poolPresenceTable, rowPools, colPools, prPools, \
                                                 pcPools, controlPools, logReadNumberRatioHistogramFitDict,
...
def FitReadNumberRatiosAndPlot(logNr2ncArray, logBins, xAxisLabel, plsq0, integrate=False):
    ...
    from utilities.pool import voigtFit
...
def CalculateRatioOfReadNumbersForSingleAddressLines(poolPresenceTable, rowPools, colPools, \
                                                     ...
                                                     logReadNumberRatioHistogramsFileName, \
                                                     fitFileName):
...
def PlotEffectOfReadThresholdOnPoolPresenceTableTaxonomy(poolPresenceTable, \
                                                         ...
                                                         startThreshold, maxThreshold, thresholdStep,
                                                         poolPresenceTaxonomyTableFileName):
    import matplotlib.pyplot as plt
    from pdb import set_trace
    from utilities.pool import CalculatePoolPresenceTableTaxonomyDict
    from utilities.output import generateOutputMatrixWithHeaders, writeOutputMatrix
    ...
    plt.title('Success of Collection Address Mapping')
    plt.show()
    ...
    plt.legend()
    plt.xlabel('Read Count Threshold')
    plt.title('Determining a Line\'s Collection Address')
    plt.show()
    ...
    plt.legend()
    plt.xlabel('Read Count Threshold')
    plt.title('Pools that Don\'t Map to Any Collection Address')
    plt.show()
...
def CalculateReadNumberRatiosForSingleAddressLines(poolPresenceTable, rowPools, colPools, prPools, \
    ...
    import pdb
    from numpy import log
    from utilities.pool import CalculateLibraryAddressesForPoolPresenceTableLine, \
        CalculatePoolCoordsForLine
